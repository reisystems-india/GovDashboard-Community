...
$report_id = 'report-' . intval($ReportConfig->getId());
$report_idgrabber = 'report-' . intval($ReportConfig->getId()) . 'grabber';
$report_style = isset($ReportConfig->options['style']) ? $ReportConfig->options['style'] : '';
...
if (isset($ReportConfig->dashboard)) {
    ...
    if (!empty($reportSize)) {
        $chartHeight = $reportSize->height - $chartTop - GD_BUILDER_BORDER_ADJUSTMENT - $chartBottom;
        $chartWidth = $reportSize->width - GD_BUILDER_BORDER_ADJUSTMENT;
        // maximum gauge display setting
        $maxWidth = $chartWidth;
    ...
    if (isset($_GET['h'])) {
        ...
        if ($_GET['q'] == 'dashboard/report/preview') {
            ...
            $chartHeight = $_GET['h'] - $chartTop - GD_BUILDER_BORDER_ADJUSTMENT - $chartBottom;
            $chartWidth = $_GET['w'] - GD_BUILDER_BORDER_ADJUSTMENT;

            // else report builder
        }
        else {
            // fix the gauge display to a reasonable value for report builder.
            $chartHeight = $_GET['h'];
            $chartWidth = $_GET['w'];
            $maxWidth = 600;
            $maxHeight = 400;
...
// maintain SQUARE ratio (w:h), but respect width to stay inside grabber
...
$pivotHeight = $chartHeight - 30;
$centerX = $chartHeight + 50;
$centerY = $chartHeight + 50;
...
foreach ($data as $name => $values) {
    ...
    if (isset($ReportConfig->options['visual']['rangeAutoMinimum'])) {
        if ($ReportConfig->options['visual']['rangeAutoMinimum'] == "Fixed" && isset($ReportConfig->options['visual']['rangeMinimum'])) {
            $gMin = $ReportConfig->options['visual']['rangeMinimum'];
        } else {
            $gMin = 0;
        }
    }
    ...
    if (isset($ReportConfig->options['visual']['rangeAutoMaximum'])) {
        if ($ReportConfig->options['visual']['rangeAutoMaximum'] == "Fixed" && isset($ReportConfig->options['visual']['rangeMaximum'])) {
            $gMax = $ReportConfig->options['visual']['rangeMaximum'];
        }
        else {
            if ($gValue < 100) {
                $gMax = 100; // auto: assume 0-100 scale of gauge
            } else {
                $gMax = (ceil($gValue / 100)) * 100 / 0.7; // auto: place value at 70% by default
            }
        }
    }
    else {
    ...
    //if value is outside the gauge range change it to min or max
    if ($gValue > $gMax) {
        $gValue = $gMax;
    } else if ($gValue < $gMin) {
        $gValue = $gMin;
    ...
    $gTickInterval = ($gMax - $gMin) / 10;

    if ($gTickInterval > 10000) {
        $gTickInterval = round($gTickInterval, -3);
    }

    if (!empty($ReportConfig->options['visual']['intervals'])) {
        $gRange = $gMax / $ReportConfig->options['visual']['intervals'];
    } else {
        $gRange = $gMax / 3;
    }

    $tickInt = $gTickInterval;

    if (!empty($ReportConfig->options['visual']['intervals'])) {

        $range = array();

        for ($i = 1; $i < $ReportConfig->options['visual']['intervals'] + 1; $i++) {
            $inter = new stdClass();
            $interFrom = 'rangeIntervalFrom' . $i;
            $interTo = 'rangeIntervalTo' . $i;
            $interColor = 'rangeIntervalColor' . $i;

            //If user enters color range less than dial minimum then change it to MIN
            $bandFrom = $ReportConfig->options['visual'][$interFrom] / 100 * $gMax;
            if ($bandFrom < $gMin) {
                $inter->from = $gMin;
            } else {
                $inter->from = $bandFrom;
            ...
            //If user enters color range more than dial maximum then change it to MAX
            $bandTo = $ReportConfig->options['visual'][$interTo] / 100 * $gMax;
            if ($bandTo > $gMax) {
                $inter->to = $gMax;
            } else {
                $inter->to = $bandTo;
            ...
            $inter->color = $ReportConfig->options['visual'][$interColor];
            $range[$i] = $inter;
        }
        ...
        foreach ($range as $key => $val) {
            $bands[] = "{ color: '$val->color', from: '$val->from', to: '$val->to' }";
        }
        $plotBands = implode($bands, ',');
    }

    $usedColumn = NULL;
    foreach ($ReportConfig->options['column_details'] as $columnDetail) {
        if ($columnDetail->name == $name) {
            $usedColumn = $columnDetail;
            $gauge_text = $columnDetail->publicName;
            break;
        }
    }
    foreach ($ReportConfig->options['column_configs'] as $columnConfig) {
        if ($usedColumn->name == $columnConfig['columnId']) {
            $gauge_text = $columnConfig['displayName'];
            break;
        }
    }

    // only first element in $data is used.
    break;
}
?>

<?php if ($isForDashboardBuilder) { ?>
    <div id="<?php echo $report_idgrabber; ?>"
         class="dashboardBuilderCanvasMover"></div>
    <div class="gd-report-resize-mask"
         style="height:<?php echo $chartHeight; ?>px;width:10px;top:<?php echo $chartTop; ?>px;left:0;"></div>
    <div class="gd-report-resize-mask"
         style="height:<?php echo $chartHeight; ?>px;width:10px;top:<?php echo $chartTop; ?>px;left:<?php echo($chartWidth - 10); ?>px;"></div>
    <div class="gd-report-resize-mask"
         style="height:10px;width:<?php echo $chartWidth; ?>px;top:<?php echo $chartTop; ?>px;left:0;"></div>
    <div class="gd-report-resize-mask"
         style="height:10px;width:<?php echo $chartWidth; ?>px;top:<?php echo($chartBottom - 10); ?>px;left:0;"></div>
<?php } ?>

    <div id="<?php echo $report_id; ?>"
         style="positon:relative;margin:0 auto;clear:both;<?php echo $chartStyle ?>" class="report report-gauge">
    ...
    <script type="text/javascript">
        (function (global, undefined) {
            (function ($, Highcharts) {
                $('#<?php echo $report_id; ?>').on('ready.report.render', function() {
                    var isForDashboardBuilder = '<?php echo $isForDashboardBuilder;?>';
                    var report_id = '<?php echo $report_id;?>';
                    var report_id_grabber = '<?php echo $report_idgrabber;?>';
                    var gMin = <?php echo $gMin;?>;
                    var gMax = <?php echo $gMax;?>;
                    var gValue = <?php echo $gValue;?>;
                    var gauge_value = '<?php echo $data['formatted_value'][$name]; ?>';
                    var gauge_text = '<?php echo $gauge_text; ?>';

                    if (isForDashboardBuilder == 'true') {
                        $("#" + report_id).mouseover(function () {
                            $("#" + report_id_grabber).show();
                        });
                    }
                    ...
                    var chart = new Highcharts.Chart({
                        ...
                        yAxis: [{
                            ...
                            tickLength: 5,
                            minorTickLength: 5,
                            //minorTickInterval: gMinorTickInterval,
                            labels: {
                                distance: 12,
                                rotation: 'auto'
                            },
                        ...
                        series: [{
                            ...
                            dataLabels: {
                                ...
                                useHTML: true,
                                formatter: function () {
                                    return '<div style="text-align: center;">' + gauge_text + ': ' + gauge_value + '</div>';
                                },
                                align: 'center',
                                y: 60,  // move label down from center pivot
                                style: {
                                    fontWeight: 'bold',
                                    fontSize: '14px'
                                },
                ...
                global.chart_<?php echo intval($ReportConfig->getId()); ?> = chart;
                });
