...
(function(global,$,undefined) {
    ...
    if ( typeof $ === 'undefined' ) {
        throw new Error('ReportColumnDisplayCreateForm requires jQuery');
    }

    if ( typeof global.GD === 'undefined' ) {
        throw new Error('ReportColumnDisplayCreateForm requires GD');
    }

    var GD = global.GD;

    var ReportColumnDisplayCreateForm = GD.View.extend({
        formContainer: null,
        ...
        loadColumns: function () {
            var reportObj = this.getController().getReport(),
                ...
                chartType = reportObj.getChartType();
            for ( var i = 0, count = columns.length; i < count; i++ ) {
                var columnDetails = reportObj.getColumn(columns[i]);
                if ( columnDetails && !reportObj.getVisual().series[columnDetails.id] && columnDetails.type != "date") {
                    if(this.getController().getReport().getChartType() !== "dynamic_text"
                        && $.inArray(columnDetails.type, this.unsupportedColumnTypeArr) === -1){
                        this.getColumnSelector().append('<a href="#" class="list-group-item" data-id="'+columnDetails.id+'">'+columnDetails.name+'</a>');
                    }else if(reportObj.getChartType() === "dynamic_text"){
                        this.getColumnSelector().append('<a href="#" class="list-group-item" data-id="'+columnDetails.id+'">'+columnDetails.name+'</a>');
                    }
                }
            }
            ...
            this.getColumnSelector().find('a').on('click',function(e){
                ...
                var column = _this.getController().getReport().getColumn($(this).data('id'));
    ...
    GD.ReportColumnDisplayCreateForm = ReportColumnDisplayCreateForm;
