...
(function(global,$,undefined) {
    ...
    if ( typeof $ === 'undefined' ) {
        throw new Error('ReportListForm requires jQuery');
    }

    if ( typeof global.GD === 'undefined' ) {
        throw new Error('ReportListForm requires GD');
    }

    var GD = global.GD;

    global.GD.ReportListForm = GD.View.extend({
        addButton: null,
        itemList: null,
        formContainer: null,
        items: null,
        ...
        enableDisableAddbutton: function(unsupportedColumnTypeArr){
            var itemsIdArr =  this.getItems().map(function(val,i,arr){ return val.columnId; });
            var reportObj = this.options.builder.getReport();
            var columns = reportObj.getColumns();
            var chartType = reportObj.getChartType();
            var allColumnsSelected = columns.every(function(val,index, arr){
                var column = reportObj.getColumn(val);
                if(column){
                    if (chartType!== "dynamic_text" && unsupportedColumnTypeArr && $.inArray(column.type,unsupportedColumnTypeArr) !== -1){
                        return true;
                    } else {
                        return $.inArray(val,itemsIdArr) !== -1;
                    }
                } else {
                    return true;
                }
            ...
            if (allColumnsSelected){
                $(this.getAddButton()).prop("disabled",true)
            } else {
                $(this.getAddButton()).prop('disabled', false);
            }
        },
        ...
        getAddButton: function() {
            if (!this.addButton) {
                this.addButton = $('<button class="btn btn-primary btn-sm" style="margin-top:10px;"><span class="glyphicon glyphicon-plus"></span> Add ' + this.getItemName() + '</button>');
            }
        ...
        attachEventHandlers: function(addHandler, clickHandler) {
            ...
            var _this = this;
            $('span.bldr-lst-itm-icn-trsh', this.getFormContainer()).click(function(e) {
                _this.removeItemClicked($(this).attr('value'));
                e.stopPropagation();
            });

            $.each(this.itemList.listItems, function(i, val){
                var listItem = val.children('div.lst-itm-val');
