...
class ReferenceLink extends AbstractObject {
    ...
    public function unregisterNestedLink(ReferenceLink $nestedLink) {
        if (!isset($this->nestedLinks[$nestedLink->linkId])) {
            throw new IllegalArgumentException(t(
                ...
                array(
                     '%datasetName' => $nestedLink->dataset->publicName, '%columnNames' => ArrayHelper::serialize($nestedLink->columnNames, ',', FALSE, FALSE),
                     '%parentDatasetName' => $this->dataset->publicName, '%parentColumnNames' => ArrayHelper::serialize($nestedLink->parentColumnNames, ',', FALSE, FALSE))));
        }
    ...
    public function assignRouteId(ReferenceRoute $route, $routeId, $level = 0) {
        ...
        $nextLevel = $level + 1;
        if (isset($route->segmentLinkIds[$nextLevel])) {
            $nestedSegmentLinkId = $route->segmentLinkIds[$nextLevel];
            $nestedLink = $this->nestedLinks[$nestedSegmentLinkId];
            $nestedLink->assignRouteId($route, $routeId, $nextLevel);
        }
    }
